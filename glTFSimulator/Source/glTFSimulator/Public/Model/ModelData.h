...
struct FModelData
{
    ...
    /** Build-derived model center stored in the .gworld directory, never in author JSON. */
    UPROPERTY()
    FVector Center = FVector::ZeroVector;

    /** Build-derived full model size stored in the .gworld directory, never in author JSON. */
    UPROPERTY()
    FVector Size = FVector::ZeroVector;
...
struct FModelMeshData
{
    ...
    /** Unscaled local-space mesh half size; the .gworld metadata member also stores full Size. */
    UPROPERTY()
...
struct GLTFSIMULATOR_API FInstancedMeshGroupInitData
{
...
/** Terminal result from one mesh-group or water-group .gworld streaming pass. */
USTRUCT(BlueprintType)
