...
public:
    ...
    /**
     * Initializes the runtime facade from tables that were already range-read on a worker thread.
     * This is the fast world-streaming path: no archive I/O is allowed here, so the game thread
     * only publishes the immutable tables and creates the lightweight node view.
     */
    bool InitializePrepared(
        const FGuid& InUUID,
