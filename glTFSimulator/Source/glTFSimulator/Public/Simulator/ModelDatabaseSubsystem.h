 ...
 *
 * Runtime mode opens WorldName.gworld and reads only its bounded directory. Authoring files are
...
public:
    ...
    /** Explicit project-build mode. Scans Projects/<Project>/resources and never opens a runtime .gworld. */
    void InitializeForAuthoringProject(
        const FString& InProjectRoot,
        FModelDatabaseReady Completion = FModelDatabaseReady());
    void Stop();

    /** Resolves a built UUID to its canonical gworld:// reference. Source mode always returns false. */
    bool Resolve(
        const FGuid& UUID,
    ...
    /** Accepts only a canonical gworld://model/<UUID> reference belonging to a mounted archive. */
    ...
    void GetSoundDefinitions(TArray<FSoundDefinition>& OutDefinitions) const;
    bool ResolveSound(const FGuid& UUID, FSoundDefinition& OutDefinition, FString& OutRuntimeReference) const;
    bool GetSoundDefinitionJson(const FGuid& UUID, FString& OutJson, FString* OutError = nullptr) const;
    ...
    /** Returns the archive that physically owns this UUID (base .gworld or an allowed external .gasset). */
    TSharedPtr<FGWorldArchiveReader, ESPMode::ThreadSafe> GetArchiveReaderForModel(const FGuid& UUID) const;
    TSharedPtr<FGWorldArchiveReader, ESPMode::ThreadSafe> GetArchiveReaderForSound(const FGuid& UUID) const;
