...
void UBuoyancyComponent::UpdateBuoyancyFromGameUpdate(float DeltaTime)
{
    ...
    // Resolve local water and the collision-free global ocean through the same common service.
    // The lowest bounds point is enough to wake buoyancy when an object first touches a surface;
    // actual forces still use the existing per-sample/bone depth and exclusion tests below.
    if (UWaterQuerySubsystem* WaterQuery = UWaterQuerySubsystem::Get(this))
    {
        ...
        const FVector LowestPoint(
            PrimitiveBounds.Origin.X,
            PrimitiveBounds.Origin.Y,
            PrimitiveBounds.Origin.Z - PrimitiveBounds.BoxExtent.Z);

        FWaterQueryResult QueryResult;
        if (WaterQuery->QueryWaterAtLocation(
                LowestPoint,
                EWaterQueryMode::Relaxed,
                EWaterQueryPurpose::Buoyancy,
                QueryResult))
        {
            EffectiveWaterLevel = bHasEffectiveWater
                ? FMath::Max(EffectiveWaterLevel, QueryResult.SurfaceZ)
                : QueryResult.SurfaceZ;
