...
USimulatorBuoyancyComponent::USimulatorBuoyancyComponent()
{
...
void USimulatorBuoyancyComponent::BeginPlay()
{
    Super::BeginPlay();
...
void USimulatorBuoyancyComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    UnregisterGameUpdate();
    Super::EndPlay(EndPlayReason);
}

void USimulatorBuoyancyComponent::RegisterGameUpdate()
{
    ...
    if (UGameUpdateSubSystem* GameUpdate = UGameUpdateSubSystem::Get(this))
    {
        GameUpdateTickHandle = GameUpdate->RegisterUpdate(
            this,
            [WeakThis = TWeakObjectPtr<USimulatorBuoyancyComponent>(this)](const float DeltaSeconds)
            {
                if (USimulatorBuoyancyComponent* StrongThis = WeakThis.Get())
                {
...
void USimulatorBuoyancyComponent::UnregisterGameUpdate()
{
    if (UGameUpdateSubSystem* GameUpdate = UGameUpdateSubSystem::Get(this))
    {
...
void USimulatorBuoyancyComponent::SetTargetComponentName(FName InTargetComponentName)
{
...
void USimulatorBuoyancyComponent::SetCommonPhysicsSettings(const FBuoyancyPhysicsSettings& InSettings)
{
    CommonPhysicsSettings = InSettings;
}

void USimulatorBuoyancyComponent::SetSkeletalMeshSettings(const FSkeletalBuoyancySettings& InSettings)
{
    SkeletalMeshSettings = InSettings;
}

UPrimitiveComponent* USimulatorBuoyancyComponent::ResolveTargetPrimitive()
{
...
void USimulatorBuoyancyComponent::ClearAutoGeneratedSamplePoints()
{
...
void USimulatorBuoyancyComponent::RebuildSamplePoints()
{
...
void USimulatorBuoyancyComponent::EnterWater(const float Level)
{
    bInWater = true;
    WaterLevel = Level;
...
void USimulatorBuoyancyComponent::ExitWater(const float Level)
{
    WaterLevel = Level;
    bInWater = false;
    UnregisterGameUpdate();
...
bool USimulatorBuoyancyComponent::ApplySkeletalMeshBuoyancy(USkeletalMeshComponent* SkeletalMesh, float DeltaTime)
{
...
void USimulatorBuoyancyComponent::UpdateBuoyancyFromGameUpdate(float DeltaTime)
{
    if (!ensureMsgf(IsInGameThread(),
        TEXT("USimulatorBuoyancyComponent::UpdateBuoyancyFromGameUpdate must run on the game thread")))
    {
        return;
    }
