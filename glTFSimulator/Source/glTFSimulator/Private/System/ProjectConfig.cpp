    ...
    EGlTFSimulatorProjectType& OutType)
{
    ...
    if (NormalizedValue.IsEmpty() || NormalizedValue == TEXT("World"))
    {
        OutType = EGlTFSimulatorProjectType::World;
        return true;
    }
    if (NormalizedValue == TEXT("Character"))
    {
        OutType = EGlTFSimulatorProjectType::Character;
        return true;
    }
    if (NormalizedValue == TEXT("Dynamic"))
    {
    ...
    FString& OutError)
{
    ...
    {
        OutConfig.WorldName = FolderFallback.TrimStartAndEnd();
        if (OutConfig.WorldName.IsEmpty())
        {
            OutError = TEXT("World project folder name is empty.");
            return false;
        }
    }
