...
namespace
{
        ...
        bool& bOutRecognized)
    {
        ...
        if (RuntimeModeKey.IsEmpty() || RuntimeModeKey == TEXT("Default"))
        {
            return ConfiguredDefault;
        }
        if (RuntimeModeKey == TEXT("Creator"))
        {
            return EPlayMode::Creator;
        }
        if (RuntimeModeKey == TEXT("RealLife"))
        {
    ...
    const AGlTFSimulatorGameplayGameModeBase* InConfigGameMode)
{
    ...
    // World environment is a rendering prerequisite and intentionally starts before .gworld I/O.
    SpawnWorldEnvManager();
            ...
            [WeakThis, StartupOwner](const bool bSuccess, const FString& Error)
            {
                ...
                if (!ReadyDatabase || !ReadyDatabase->IsBuiltWorld())
                {
                    StrongThis->FailWorldStartup(
                        TEXT("World startup failed: runtime uses Worlds/*.gworld only. Build the project first from the Projects UI in MainWorld."));
                    return;
                }
...
void UGameManagerSubSystem::ContinueWorldStartupAfterDatabase()
{
    ...
    {
        FailWorldStartup(
            TEXT("World startup failed: no verified .gworld archive is open."));
        return;
    }
...
void UGameManagerSubSystem::ValidatePostLoadGameplayLifecycle(UWorld* LoadedWorld)
{
    ...
    // A world that resolves to a real .gworld but does not use the gameplay GameMode classes is a
    ...
    {
        UE_LOG(LogTemp, Error,
            TEXT("Gameplay world '%s' resolved .gworld '%s' but its active GameMode is not derived from AGlTFSimulatorGameplayGameModeBase. Assign SingleplayGameMode or MultiplayGameMode in World Settings / travel override."),
...
void UGameManagerSubSystem::InitializeRuntimeWorldState()
{
    ...
    // Loading UI, world data, world rendering, water, and .gworld streaming now start from one owner.
    ShowLoadingWidget();
    LoadWorldData();
}
...
void UGameManagerSubSystem::LoadWorldData()
{
    ...
    {
        FailWorldStartup(TEXT("World startup failed: no .gworld reader is available for config.json."));
        return;
    }
    TWeakObjectPtr<UGameManagerSubSystem> WeakThis(this);
    const bool bQueued = FSafeFileIO::RunTrackedWorker([WeakThis, ArchiveReader, RequestedRoot, StartupOwner]()
    {
        ...
        {
            Config = FSafeFileIO::ParseJsonText(ConfigText, TEXT(".gworld/config.json"), Limits);
        }
        else
        {
...
void UGameManagerSubSystem::StartGameplayWorldStreaming(const FString& InWorldRoot, const FString& InInitialPlayerName)
{
    ...
    // StartWorldStreaming deliberately refuses every non-.gworld source. Convert that
        ...
        || !StreamSubSystem->IsActiveForWorld(World))
    {
        StreamSubSystem = nullptr;
        FailWorldStartup(
            TEXT("World startup failed: the verified .gworld streaming session could not be started."));
    }
}
...
void UGameManagerSubSystem::ClearTransientRuntimeReferences()
{
    ...
    PendingWorldBuildSounds.Empty();
    PendingWorldBuildSoundJson.Empty();
    ...
    TSoftObjectPtr<UWorld> MainWorld)
{
    ...
    // Editor-only transaction cleanup is handled centrally by glTFSimulatorEditor::PreLoadMap.
...
void UGameManagerSubSystem::GetProjectSummaries(
    TArray<FGlTFSimulatorProjectSummary>& OutProjects) const
{
    check(IsInGameThread());
    OutProjects.Reset();
    ...
    OutProjects.Reserve(ProjectFolders.Num());
    for (const FString& Folder : ProjectFolders)
    {
        FString SafeName;
        if (!TryNormalizeWorldFolderName(Folder, SafeName, false) || SafeName != Folder)
        {
            continue;
        }
        const FString ProjectRoot = FPaths::Combine(PATH_PROJECTS, SafeName);
        const FString ConfigPath = FPaths::Combine(ProjectRoot, LEVEL_FILE_NAME);
        const FString ResourcesPath = FPaths::Combine(ProjectRoot, DIRECTORY_PROJECT_RESOURCES);
        ...
        FString Error;
        if (!GlTFSimulatorProjectConfig::Load(ConfigPath, SafeName, Config, nullptr, Error))
        {
            UE_LOG(LogTemp, Warning,
                TEXT("Project '%s' ignored because config.json is invalid: %s"),
                *SafeName, *Error);
            continue;
        }

        FGlTFSimulatorProjectSummary& Summary = OutProjects.AddDefaulted_GetRef();
        Summary.FolderName = SafeName;
        Summary.DisplayName = Config.GetDisplayName(SafeName);
        Summary.ProjectType = Config.ProjectType;
        Summary.bAllowExternalAssets = Config.bAllowExternalAssets;
    }
}
...
bool UGameManagerSubSystem::CreateProjectByName(const FString& ProjectName)
{
    check(IsInGameThread());
    if (IsProjectBuildInProgress())
    {
        LastSaveMessage = TEXT("A project cannot be created while another build is active.");
        NotifyStateChanged();
        return false;
    }

    FString SafeName;
    if (!TryNormalizeWorldFolderName(ProjectName, SafeName, false) || SafeName != ProjectName.TrimStartAndEnd())
    {
        LastSaveMessage = TEXT("Project name must be a safe single folder name.");
        NotifyStateChanged();
        return false;
    }
    ...
    const FString ResourcesRoot = FPaths::Combine(ProjectRoot, DIRECTORY_PROJECT_RESOURCES);
    const FString ConfigPath = FPaths::Combine(ProjectRoot, LEVEL_FILE_NAME);
    if (ProjectRoot.IsEmpty() || FileManager.DirectoryExists(*ProjectRoot) || FileManager.FileExists(*ConfigPath))
    {
        LastSaveMessage = TEXT("A project with that name already exists.");
        NotifyStateChanged();
        return false;
    }
    if (!FileManager.MakeDirectory(*ResourcesRoot, true))
    {
        LastSaveMessage = TEXT("Could not create the project resources directory.");
        NotifyStateChanged();
        return false;
    }
    ...
    const FSafeFileWriteResult Saved = FSafeFileIO::SaveJsonBlocking(Json, ConfigPath);
    if (!Saved.IsSuccess())
    {
        FileManager.DeleteDirectory(*ProjectRoot, false, true);
        LastSaveMessage = FString::Printf(TEXT("Project config.json creation failed: %s"), *Saved.Error);
        NotifyStateChanged();
        return false;
    }

    LastSaveMessage = FString::Printf(TEXT("Project created: %s"), *SafeName);
    NotifyStateChanged();
    return true;
}
...
bool UGameManagerSubSystem::BuildProjectByName(const FString& ProjectName)
{
        ...
        [WeakThis, ProjectRoot, SafeName, ConfigPath]()
    {
        ...
        else
        {
            ...
            {
                // Canonicalize authoring defaults into the immutable build. Runtime never replays
                // authoring fallback rules, so an omitted ProjectName/WorldName cannot make a valid
                // project disappear from the Worlds list after it has been built.
                TSharedPtr<FJsonObject> Canonical = ParsedConfig.JsonObject;
                Canonical->SetStringField(
                    PROJECT_TYPE_FIELD,
                    GlTFSimulatorProjectConfig::ToString(ProjectConfig.ProjectType));
                Canonical->SetStringField(PROJECT_NAME_FIELD, ProjectConfig.ProjectName);
                Canonical->SetBoolField(ALLOW_EXTERNAL_ASSETS_FIELD, ProjectConfig.bAllowExternalAssets);
                if (ProjectConfig.ProjectType == EGlTFSimulatorProjectType::World)
                {
                    Canonical->SetStringField(CONFIG_WORLD_NAME_FIELD, ProjectConfig.WorldName);
                }
                else
                {
                    Canonical->RemoveField(CONFIG_WORLD_NAME_FIELD);
                }

                const TSharedRef<TJsonWriter<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>> Writer =
                    TJsonWriterFactory<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>::Create(&ConfigJson);
                if (!FJsonSerializer::Serialize(Canonical.ToSharedRef(), Writer) || ConfigJson.IsEmpty())
                {
                    ValidationError = TEXT("Project config.json could not be canonicalized for the archive.");
                }
            }
        }
...
void UGameManagerSubSystem::BakeWorldData()
{
    ...
    ModelDatabase->GetSoundDefinitions(PendingWorldBuildSounds);
    PendingWorldBuildSoundJson.Reserve(PendingWorldBuildSounds.Num());
    for (const FSoundDefinition& Sound : PendingWorldBuildSounds)
    {
        FString SoundJson;
        FString SoundError;
        if (!ModelDatabase->GetSoundDefinitionJson(Sound.UUID, SoundJson, &SoundError) || SoundJson.IsEmpty())
        {
            CompleteWorldArchiveBuild(false, FString(), FString::Printf(
                TEXT("Sound definition could not be prepared. Sound=%s Reason=%s"),
                *Sound.DisplayName, *SoundError));
            return;
        }
        PendingWorldBuildSoundJson.Add(Sound.UUID, MoveTemp(SoundJson));
    }
    ...
    LastSaveMessage = FString::Printf(
        TEXT("%s project build started: %d model(s), %d sound(s)"),
        *GlTFSimulatorProjectConfig::ToString(ActiveBuildProjectType),
        WorldBakeTotalModels, PendingWorldBuildSounds.Num());
    NotifyStateChanged();
    ...
    else if (!PendingWorldBuildSounds.IsEmpty())
    {
        FinishWorldBake();
    }
    else
    {
        CompleteWorldArchiveBuild(false, FString(),
            TEXT("recursive resources scan produced zero buildable Model or Sound assets"));
    }
}
...
void UGameManagerSubSystem::StartNextWorldBakeModel()
{
    ...
    while (WorldBakeNextModelIndex < PendingWorldBakeModels.Num())
    {
        ...
        if (Definition.ModelType == EModelDefinitionType::Character)
        {
            ...
            if (!IsValid(CharacterSkeleton) && IsValid(CharacterSkeletalMesh))
            {
                CharacterSkeleton = CharacterSkeletalMesh->GetSkeleton();
            }
            if (!IsValid(CharacterMaterial) || !IsValid(CharacterSkeleton))
            {
                ...
                ++WorldBakeFailedModels;
                LastSaveMessage = FString::Printf(
                    TEXT("Character build requires DefaultCharacterMaterial and DefaultCharacterSkeleton in AssetRegistry: %s"),
                    *FPaths::GetCleanFilename(ModelPath));
                UE_LOG(LogTemp, Error, TEXT("%s"), *LastSaveMessage);
                ActiveWorldBuildTask = nullptr;
...
void UGameManagerSubSystem::FinishWorldBake()
{
    ...
    LastSaveMessage = bWorldProject
        ? TEXT("Writing and verifying the world .gworld archive...")
    ...
    TArray<FSoundDefinition> SoundDefinitions = MoveTemp(PendingWorldBuildSounds);
    TMap<FGuid, FString> SoundDefinitionJson = MoveTemp(PendingWorldBuildSoundJson);
    TWeakObjectPtr<UGameManagerSubSystem> WeakThis(this);
    ...
    const bool bQueued = FSafeFileIO::RunTrackedWorker(
        [WeakThis, OutputRootOrPath, ArchiveConfigJson, BuildGeneration, Cancellation,
            bWorldProject, BuildModels = MoveTemp(BuildModels), SoundDefinitions = MoveTemp(SoundDefinitions),
            SoundDefinitionJson = MoveTemp(SoundDefinitionJson)]() mutable
    {
        FString ArchivePath;
        FString Error;
        ...
        TArray<FGWorldBuildSound> BuildSounds;
        BuildSounds.Reserve(SoundDefinitions.Num());
        for (const FSoundDefinition& Sound : SoundDefinitions)
        {
            if (ShouldCancel()) { Error = TEXT("project build cancelled while preparing Sound assets"); break; }
            const FString* Json = SoundDefinitionJson.Find(Sound.UUID);
            const int64 SourceSize = IFileManager::Get().FileSize(*Sound.WavPath);
            const FDateTime SourceTime = IFileManager::Get().GetTimeStamp(*Sound.WavPath);
            if (!Json || Json->IsEmpty() || SourceSize <= 0 || SourceSize > FGWorldArchive::MaxSoundBytes)
            {
                Error = FString::Printf(TEXT("Sound source metadata is invalid or exceeds the size limit: %s"),
                    *Sound.WavPath);
                break;
            }
            FGWorldBuildSound& Built = BuildSounds.AddDefaulted_GetRef();
            Built.Definition = Sound;
            Built.DefinitionJson = *Json;
            Built.SourceFileSize = SourceSize;
            Built.SourceTimestamp = SourceTime;
        }
        const bool bSuccess = Error.IsEmpty() && (bWorldProject
            ? FGWorldArchive::BuildBlocking(
                OutputRootOrPath, BuildModels, BuildSounds, ArchivePath, Error, ShouldCancel, ArchiveConfigJson)
