    ...
    const bool bInRenderOnlyStreaming)
{
        ...
        || Database->GetWorldRoot() != WorldRoot)
    {
        ...
        bStartupFailed = true;
        WriteLogAsync(TEXT("Streaming startup rejected: a verified .gworld archive is not open"));
        return;
    }
...
void UWorldSceneStreamingSubsystem::UpdateStreaming()
{
    ...
    // Only the nearest SpawnBudget records can be spawned this tick. Keep that bounded top-K set
    // instead of allocating and sorting every in-range scene (which can be tens of thousands in a
    // large archive). PendingLoadCount still tracks whether startup needs another burst.
    ...
    for (const FWorldSceneStreamRecord& Record : SceneRecords)
    {
        ...
        if (IsInsideRange(Record.Bounds, 1.0f))
        {
            ...
            if (PendingLoads.Num() < SpawnBudget)
            {
                ...
                Candidate.DistanceSq = CandidateDistanceSq;
            }
            else
            {
                // SpawnBudget is intentionally small (normally a few dozen), so a bounded linear
                // farthest search is faster and simpler than maintaining a large all-record sort.
                int32 FarthestIndex = 0;
                double FarthestDistanceSq = PendingLoads[0].DistanceSq;
                for (int32 PendingIndex = 1; PendingIndex < PendingLoads.Num(); ++PendingIndex)
                {
                    if (PendingLoads[PendingIndex].DistanceSq > FarthestDistanceSq)
                    {
                        FarthestIndex = PendingIndex;
                        FarthestDistanceSq = PendingLoads[PendingIndex].DistanceSq;
                    }
                }
                if (CandidateDistanceSq < FarthestDistanceSq)
                {
                    PendingLoads[FarthestIndex].Record = &Record;
                    PendingLoads[FarthestIndex].DistanceSq = CandidateDistanceSq;
                }
            }
        }
        else if (Existing)
    ...
    bInitialScenePassComplete = PendingLoadCount <= SpawnBudget;
    if (!bInitialScenePassComplete)
    {
...
void UWorldSceneStreamingSubsystem::BeginInitialPlayerStreamingIfNeeded()
{
    ...
    // Character loading is independent from static scene range reads/finalization. Starting it as
    // soon as the immutable directory has been enumerated hides most character decode/build time
    // behind the initial world stream. Final player activation still waits for AreInitialModelsReady,
    // so this changes latency only, not startup correctness or collision/streaming readiness.
...
bool UWorldSceneStreamingSubsystem::ResolveInitialCharacterIndex()
{
    ...
    const FString CandidateSelector = InitialPlayerName.TrimStartAndEnd();
    ...
    WriteLogAsync(FString::Printf(
        TEXT("Selected character was not found in .gworld; using the default pawn. Selector=%s"),
        *InitialPlayerName));
    return false;
}
...
void UWorldSceneStreamingSubsystem::HandlePlayerLoadFailure(const FString& Reason)
{
    ...
    {
        // .gworld is immutable for this session, so a failing member cannot become healthy due to
