...
void AEditableMeshActor::BeginObject(const FString& InObjectName)
{
    ObjectName = InObjectName.IsEmpty() ? TEXT("GeneratedMesh") : InObjectName;
    BaseName = ObjectName;
    FString IgnoredSuffix;
    ObjectName.Split(TEXT(";"), &BaseName, &IgnoredSuffix, ESearchCase::CaseSensitive, ESearchDir::FromStart);
    bFinalized = false;
...
void AEditableMeshActor::BeginEditingExistingObject()
{
    bFinalized = false;
...
void AEditableMeshActor::ClearMesh()
{
    Vertices.Empty();
    Triangles.Empty();
    CurrentFace.Empty();
...
int32 AEditableMeshActor::AddVertexWorld(const FVector& WorldLocation)
{
...
bool AEditableMeshActor::AddExistingVertexToCurrentFace(int32 VertexIndex)
{
...
bool AEditableMeshActor::MoveVertexWorld(int32 VertexIndex, const FVector& WorldLocation)
{
...
bool AEditableMeshActor::StartConnectedVertexFromIndex(int32 SourceVertexIndex)
{
...
void AEditableMeshActor::ClearConnectedVertexSource()
{
...
FVector AEditableMeshActor::GetVertexWorldLocation(int32 VertexIndex) const
{
...
void AEditableMeshActor::SetPreviewVertexWorld(const FVector& WorldLocation)
{
    if (bFinalized)
    {
        return;
...
void AEditableMeshActor::ClearPreviewVertex()
{
    bHasPreviewVertex = false;
    PreviewVertex = FVector::ZeroVector;
    RebuildPreviewMesh();
...
void AEditableMeshActor::SetEditPreviewVisible(bool bVisible)
{
    bShowEditPreview = bVisible;
    RebuildPreviewMesh();
}

void AEditableMeshActor::SetHighlightedVertexIndex(int32 VertexIndex, bool bMoving)
{
...
bool AEditableMeshActor::CanAppendVertexToCurrentFace(int32 VertexIndex, bool& bOutClosedToFirstVertex) const
{
...
int32 AEditableMeshActor::ClampCurrentFaceTriangleStartIndex(const TArray<int32>& TriangleArray) const
{
...
void AEditableMeshActor::AddOrUpdateFaceForArrays(TArray<int32>& InOutTriangles, const TArray<int32>& Face, int32 ReplaceFromIndex) const
{
...
void AEditableMeshActor::AddOrUpdateFace()
{
...
bool AEditableMeshActor::HasDanglingPointOrLineEdit() const
{
...
bool AEditableMeshActor::DiscardDanglingPointOrLineEdit()
{
...
bool AEditableMeshActor::CanFinalizeAsObject() const
{
...
bool AEditableMeshActor::IsTopologyValid() const
{
    return IsTopologyValidFor(Vertices, Triangles);
}

void AEditableMeshActor::FinalizeObject()
{
    bFinalized = true;
    bHasPreviewVertex = false;
    PreviewVertex = FVector::ZeroVector;
...
void AEditableMeshActor::RebuildMesh(bool bCreateCollision)
{
    if (!IsValid(MeshComponent))
    {
        return;
...
void AEditableMeshActor::AppendPreviewBox(TArray<FVector>& OutVertices, TArray<int32>& OutTriangles, TArray<FColor>& OutColors, const FVector& Center, const FVector& Extent, const FColor& Color) const
{
...
void AEditableMeshActor::AppendPreviewSegment(TArray<FVector>& OutVertices, TArray<int32>& OutTriangles, TArray<FColor>& OutColors, const FVector& Start, const FVector& End, float Thickness, const FColor& Color) const
{
...
void AEditableMeshActor::AppendPreviewEdgeIfValid(TSet<FIntPoint>& EdgeSet, int32 A, int32 B) const
{
...
void AEditableMeshActor::RebuildPreviewMesh()
{
    if (!IsValid(PreviewComponent))
    {
        return;
...
FVector AEditableMeshActor::GetLastVertexWorld() const
{
    if (Vertices.Num() == 0)
    {
        return GetActorLocation();
...
FVector AEditableMeshActor::GetPreviewVertexWorld() const
{
    if (!bHasPreviewVertex)
    {
...
bool AEditableMeshActor::FindNearestVertexToRay(const FVector& RayStart, const FVector& RayDirection, float MaxDistance, int32& OutVertexIndex, FVector& OutVertexWorldLocation, float& OutDistance) const
{
...
FGeneratedMeshRecord AEditableMeshActor::ToGeneratedMeshRecord() const
{
