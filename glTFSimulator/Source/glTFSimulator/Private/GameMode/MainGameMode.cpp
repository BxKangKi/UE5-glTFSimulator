...
void AMainGameMode::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    ...
    // cleanup is owned centrally by the glTFSimulatorEditor module.
...
void AMainGameMode::BuildLevelFolderNameMap()
{
    ...
    IFileManager::Get().FindFiles(WorldFiles,
        *FPaths::Combine(PATH_WORLDS, TEXT("*.gworld")), true, false);
    ...
    for (const FString& FileName : WorldFiles)
    {
        ...
        {
            UE_LOG(LogTemp, Error,
                TEXT("[WorldSelection] Ignoring invalid .gworld '%s': %s"), *ArchivePath, *Error);
            continue;
        }
        ...
        {
            UE_LOG(LogTemp, Error,
                TEXT("[WorldSelection] Ignoring .gworld without a valid config WorldName: %s"), *ArchivePath);
            continue;
        }
