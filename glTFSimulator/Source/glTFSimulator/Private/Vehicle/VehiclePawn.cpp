...
bool AVehiclePawn::LoadVehicleModel(const FString& InModelReference, const FString& InObjectName)
{
    ...
    const FSafeJsonLoadResult TuningJson = FSafeFileIO::ParseJsonText(
        Model.DefinitionJson, TEXT(".gworld vehicle definition"), TuningLimits);
    ...
    auto PrepareLoadedVehicleState = [this]()
    {
        ...
        // Normalize the chassis pose before enabling collision/physics. Otherwise an imported/spawned
        // pitch or roll can preload one side of the suspension during Chaos' first step.
        if (!UMultiplayerWorldSubSystem::ShouldUseClientRenderOnlyStreaming(this) || HasAuthority())
        {
            ResetVehiclePoseAboveGround();
        }
...
void AVehiclePawn::ResetVehiclePoseAboveGround()
{
    ...
    // Freeze Chaos before teleporting the chassis. A small authored/spawned pitch or roll can put
    // one suspension side under load before the opposite wheels have even completed their first
    // ground query, which makes a freshly spawned vehicle visibly lean to one side.
    Body->SetSimulatePhysics(false);
    Body->SetEnableGravity(false);
    Body->SetPhysicsLinearVelocity(FVector::ZeroVector);
    Body->SetPhysicsAngularVelocityInRadians(FVector::ZeroVector);
    ...
    // A center trace alone is not enough on sloped/uneven ground: one wheel can start deeply
    // compressed while its opposite side is near rest length.  Use every wheel's ground sample to
    // lift the upright chassis to the minimum Z where no valid wheel starts more compressed than its
    // target spring length.  This prevents the first Chaos solve from creating a one-sided impulse.
    if (!WheelOffsets.IsEmpty())
    {
        const FTransform ProvisionalBodyTransform = Body->GetComponentTransform();
        const float RequiredInitialNormalZ = FMath::Clamp(MinSuspensionHitNormalDot, 0.0f, 1.0f);
        FCollisionQueryParams WheelHeightQueryParams(SCENE_QUERY_STAT(VehicleInitialWheelHeightTrace), false, this);
        WheelHeightQueryParams.AddIgnoredActor(this);

        float RequiredActorZ = ProvisionalTargetZ;
        bool bFoundWheelGround = false;
        for (int32 WheelIndex = 0; WheelIndex < WheelOffsets.Num(); ++WheelIndex)
        {
            const float TargetSpringLength = GetTargetWheelSpringLength(WheelIndex);
            ...
            RequiredActorZ = FMath::Max(RequiredActorZ, WheelRequiredActorZ);
            bFoundWheelGround = true;
        }

        if (bFoundWheelGround && RequiredActorZ > ProvisionalTargetZ + KINDA_SMALL_NUMBER)
        {
            // Only lift here; never force the vehicle downward based on one noisy wheel trace.
            // The suspension/gravity pass will settle any deliberately retained clearance.
    ...
    // Seed suspension from the final upright pose before Chaos wakes up. Starting all wheels at an
    // arbitrary target length makes the first solver frame depend on trace/update order; one side can
    // receive a large compression impulse before the opposite side has a valid history.
    const FTransform InitialBodyTransform = Body->GetComponentTransform();
    const float RequiredInitialNormalZ = FMath::Clamp(MinSuspensionHitNormalDot, 0.0f, 1.0f);
    FCollisionQueryParams WheelInitQueryParams(SCENE_QUERY_STAT(VehicleInitialWheelGroundTrace), false, this);
    WheelInitQueryParams.AddIgnoredActor(this);
    ...
    for (int32 WheelIndex = 0; WheelIndex < WheelOffsets.Num(); ++WheelIndex)
    {
        ...
        if (bHitGround)
        {
            ...
            if (RawSpringLength <= MaxSpringLength + 3.5f)
            {
                ...
                bInitiallyGrounded = true;
                ++GroundedWheelCount;
            }
        }

        WheelSpringLengths[WheelIndex] = InitialSpringLength;
        WheelVisualSpringLengths[WheelIndex] = InitialSpringLength;
        WheelSuspensionForces[WheelIndex] = 0.0f;
        WheelLateralForces[WheelIndex] = 0.0f;
        WheelGrounded[WheelIndex] = bInitiallyGrounded;
    }
    ...
    {
        WheelSuspensionForces[WheelIndex] = WheelGrounded[WheelIndex] ? ResetSupportForce : 0.0f;
    }

    // Reapply mass/COM/damping only after the upright transform and symmetric suspension state are
    // established. This is the point at which Chaos is allowed to simulate again.
    ApplyVehicleBodyPhysicsSettings();
