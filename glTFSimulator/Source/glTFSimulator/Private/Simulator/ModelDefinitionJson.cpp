...
namespace
{
    bool WriteGeneratedDefinitionAtomically(const FString& GlbPath, const FString& JsonPath)
    {
        ...
        RootObject->SetStringField(TEXT("AssetType"), TEXT("Model"));
    ...
    TArray<FString>* OutDiscoveredGlbFiles)
{
    ...
    constexpr int32 MaxGeneratedDefinitions = 100000; // Must not exceed the .gworld model cap.
...
{
    return ModelType == TEXT("Static")
        || ModelType == TEXT("Dynamic")
        || ModelType == TEXT("Character");
}
    ...
    FString* OutCanonicalJson)
{
    ...
    FString TypeText;
    Root->TryGetStringField(TEXT("AssetType"), AssetTypeText);
    AssetTypeText.TrimStartAndEndInline();
    if (AssetTypeText.IsEmpty())
    {
        AssetTypeText = TEXT("Model");
        Root->SetStringField(TEXT("AssetType"), AssetTypeText);
    }
    if (AssetTypeText != TEXT("Model"))
    {
