...
namespace RuntimeModelResolverPrivate
{
        ...
        const TSharedPtr<FGWorldArchiveReader, ESPMode::ThreadSafe>& ArchiveReader)
    {
        return FString::Printf(
            ...
            *ArchiveReader->GetBuildId().ToString(EGuidFormats::Digits),
    ...
    FString& OutError)
{
    ...
    {
        OutError = TEXT("a verified .gworld model database is not open");
        return false;
    }
...
UWorldBakedModelAsset* FRuntimeModelResolver::LoadAssetFromPreparedTables(
    const FGuid& UUID,
    const FString& Reference,
    const TSharedPtr<FGWorldArchiveReader, ESPMode::ThreadSafe>& ArchiveReader,
    ...
    FString& OutError)
{
    ...
    if (!UUID.IsValid()
        ...
        || !FGWorldArchive::ParseModelReference(Reference, ParsedUUID)
        || ParsedUUID != UUID)
    {
        OutError = TEXT("cannot publish invalid prepared built-model tables");
        return nullptr;
    }
    ...
    TMap<FString, TWeakObjectPtr<UWorldBakedModelAsset>>& FacadeCache = GetFacadeCache();
    PruneFacadeCache(FacadeCache);
    const FString CacheKey = MakeFacadeCacheKey(UUID, ArchiveReader);
    if (TWeakObjectPtr<UWorldBakedModelAsset>* Cached = FacadeCache.Find(CacheKey))
    {
        if (UWorldBakedModelAsset* Existing = Cached->Get(); IsValid(Existing))
        {
            ...
            OutError))
    {
        if (OutError.IsEmpty())
        {
            OutError = TEXT("could not initialize the prepared baked model facade");
        }
        return nullptr;
    }
