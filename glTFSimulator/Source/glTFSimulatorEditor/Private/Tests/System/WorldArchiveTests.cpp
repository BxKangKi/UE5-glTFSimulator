...
{
    void AppendU16(TArray<uint8>& Bytes, const uint16 Value)
    {
    ...
    /** Small mono 16-bit PCM RIFF/WAVE used to verify Sound archive members. */
    TArray<uint8> MakeMinimalPcmWav()
    {
        constexpr uint16 Channels = 1;
        constexpr uint32 SampleRate = 8000;
        constexpr uint16 BitsPerSample = 16;
        const int16 Samples[] = {0, 1000, -1000, 0};
        const uint32 DataBytes = sizeof(Samples);
        ...
        Bytes.Append(reinterpret_cast<const uint8*>(Samples), DataBytes);
        return Bytes;
    }
...
bool FWorldFolderNameValidationTest::RunTest(const FString& Parameters)
{
    ...
    // Current persisted entity rows use fixed toolbar wire values; keep the serialization contract explicit.
    TestEqual(TEXT("Static toolbar wire value"),
        static_cast<uint8>(EToolbarItemKind::Static), static_cast<uint8>(2));
    TestEqual(TEXT("Weapon toolbar wire value"),
        static_cast<uint8>(EToolbarItemKind::Weapon), static_cast<uint8>(3));
    TestEqual(TEXT("Vehicle toolbar wire value"),
...
bool FRecursiveWorldSourceDiscoveryTest::RunTest(const FString& Parameters)
{
    ...
    // ModelType may be omitted; the current schema normalizes an untyped GLB to Static.
    ...
    const FString UntypedDefinition = FString::Printf(
        TEXT("{\"UUID\":\"%s\",\"Name\":\"direct\",\"DisplayName\":\"Direct\"}"),
    ...
    TestTrue(TEXT("Untyped Model definition remains buildable"),
        ModelDefinitionJson::LoadDefinition(
            DirectJson, DirectGlb, ParsedDefinition, ParseError, &CanonicalJson));
    TestTrue(TEXT("Untyped definition defaults AssetType to Model"),
        ParsedDefinition.AssetType == EAssetDefinitionType::Model);
    TestTrue(TEXT("Untyped definition normalizes to Static"),
        ParsedDefinition.ModelType == EModelDefinitionType::Static);
    TestTrue(TEXT("Archive snapshot contains canonical AssetType"),
        CanonicalJson.Contains(TEXT("\"AssetType\":\"Model\"")));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGWorldArchiveRoundTripTest,
    "glTFSimulator.Streaming.GworldRoundTrip",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGWorldArchiveRoundTripTest::RunTest(const FString& Parameters)
{
    (void)Parameters;
    using namespace WorldArchiveTestsPrivate;
    const FString Sandbox = MakeTestRoot(TEXT("Gworld"));
    ...
    // materialize every model member merely because they share one .gworld container.
    ...
    const FString SoundDirectory = FPaths::Combine(
        Sandbox, TEXT("Projects"), TEXT("TestWorld"), TEXT("resources"), TEXT("audio"));
    TestTrue(TEXT("Create sound source directory"),
        IFileManager::Get().MakeDirectory(*SoundDirectory, true));
    const FString WavPath = FPaths::Combine(SoundDirectory, TEXT("beep.wav"));
    const TArray<uint8> SourceWave = MakeMinimalPcmWav();
    TestTrue(TEXT("Write paired Sound WAV"), FFileHelper::SaveArrayToFile(SourceWave, *WavPath));

    FGWorldBuildSound Sound;
    Sound.Definition.AssetType = EAssetDefinitionType::Sound;
    Sound.Definition.UUID = FGuid::NewGuid();
    Sound.Definition.Name = TEXT("beep");
    Sound.Definition.DisplayName = TEXT("Beep");
    Sound.Definition.WavPath = WavPath;
    Sound.Definition.JsonPath = FPaths::ChangeExtension(WavPath, TEXT("json"));
    Sound.DefinitionJson = FString::Printf(
        TEXT("{\"AssetType\":\"Sound\",\"UUID\":\"%s\",\"Name\":\"beep\",\"DisplayName\":\"Beep\"}"),
        *Sound.Definition.UUID.ToString(EGuidFormats::DigitsWithHyphensLower));
    Sound.SourceFileSize = IFileManager::Get().FileSize(*WavPath);
    Sound.SourceTimestamp = IFileManager::Get().GetTimeStamp(*WavPath);

    TArray<FGWorldBuildModel> Models;
    Models.Add(Model);
    TArray<FGWorldBuildSound> Sounds;
    Sounds.Add(Sound);
    FString ArchivePath;
    FString Error;
    ...
    TestTrue(TEXT("Build immutable archive"),
        FGWorldArchive::BuildBlocking(
            Root, Models, Sounds, ArchivePath, Error, TFunction<bool()>(), WorldConfigJson));
    ...
    TestEqual(TEXT("Directory sound count"), SoundRecords.Num(), 1);
    FString SoundJson;
    TestTrue(TEXT("Range-read Sound definition"),
        Reader->ReadSoundDefinition(Sound.Definition.UUID, SoundJson, Error));
    TestEqual(TEXT("Sound definition JSON round-trips"), SoundJson, Sound.DefinitionJson);
    TArray<uint8> ArchivedWave;
    TestTrue(TEXT("Range-read Sound WAV"),
        Reader->ReadSoundWave(Sound.Definition.UUID, ArchivedWave, Error));
    TestTrue(TEXT("Sound WAV bytes round-trip"), ArchivedWave == SourceWave);
    ...
    TestTrue(TEXT("Delete source WAV after build"), IFileManager::Get().Delete(*WavPath));
    Bundle.Reset();
    ...
    TestFalse(TEXT("Existing build blocks source rebuild"),
        FGWorldArchive::BuildBlocking(Root, Models, Sounds, RebuildPath, RebuildError));
    return true;
}
